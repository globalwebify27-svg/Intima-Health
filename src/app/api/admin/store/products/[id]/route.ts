...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await connectDB();
    const updated = await ProductModel.findByIdAndUpdate((await params).id, body, { new: true });
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await connectDB();
    const deleted = await ProductModel.findByIdAndUpdate((await params).id, { deletedAt: new Date() }, { new: true });
